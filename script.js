...
function salvarDados(chave, dados) {
  ...
  function atualizarResumo() {
    ...
    document.getElementById('resumoValores').innerHTML = `
      ...
      <hr>
      <b>DINHEIRO DO MÊS (SALDO ANTERIOR + SALÁRIO + BÔNUS):</b> R$ ${dinheiroMes.toFixed(2)} <button type="button" class="btn btn-link p-0 ms-1" onclick="mostrarAjuda('dinheiroMes')" title="Clique para mais informações" style="color: #0d6efd; text-decoration: none; font-size: 1.1rem;">❓</button><br>
      <b>DESPESAS FIXAS:</b> R$ ${totalDespesasFixas.toFixed(2)}<br>
      <b>DÉBITO:</b> R$ ${totalDebito.toFixed(2)}<br>
      <b>CARTÃO:</b> R$ ${totalCartao.toFixed(2)}<br>
      <b>FIXOS + DÉBITO + CARTÃO:</b> R$ ${fixosMaisDebitoMaisCartao.toFixed(2)}<br>
      <b>DINHEIRO EM CONTA (DINHEIRO DO MÊS - FIXOS - DÉBITO):</b> R$ ${dinheiroConta.toFixed(2)} <button type="button" class="btn btn-link p-0 ms-1" onclick="mostrarAjuda('dinheiroConta')" title="Clique para mais informações" style="color: #0d6efd; text-decoration: none; font-size: 1.1rem;">❓</button><br>
      <b>SOBRA (DINHEIRO EM CONTA - CARTÃO):</b> <span style="color:blue"><b>R$ ${sobra.toFixed(2)}</b></span> <button type="button" class="btn btn-link p-0 ms-1" onclick="mostrarAjuda('sobra')" title="Clique para mais informações" style="color: #0d6efd; text-decoration: none; font-size: 1.1rem;">❓</button><br>
    `;
  }
  ...
  function mostrarAjuda(tipo) {
    ...
    const explicacoes = {
      ...
      'dinheiroConta': 'Dinheiro em conta = Dinheiro do Mês - Despesas Fixas - Débito. Representa o valor que sobra na conta após pagar todas as despesas fixas e compras no débito.',
      'sobra': 'Sobra = Dinheiro em Conta - Cartão. É o valor que realmente sobra após pagar todas as despesas e reservar o valor do cartão para o próximo mês.'
    };
  ...
  function atualizarGraficoComDados(dados) {
    ...
    const considerarFixas = document.getElementById('considerarFixasGrafico') ? document.getElementById('considerarFixasGrafico').checked : true;
    let dadosFiltrados = [];
    
    // Coletar dados baseado no filtro
    if ((filtroTipo === 'todos' || filtroTipo === 'despesasFixas') && considerarFixas) {
  ...
  window.onload = function() {
    carregarTudo();
    atualizarTabelaComparacao = (function(orig) {
      return function() {
        orig();
  ...
  // Sincronizar seletores de mês entre planilha e gráfico
  function sincronizarMesesPlanilhaEGrafico(novoMes) {
    const seletorPlanilha = document.getElementById('seletorMes');
    const seletorGraficos = document.getElementById('seletorMesGraficos');
    if (seletorPlanilha && seletorPlanilha.value !== novoMes) seletorPlanilha.value = novoMes;
    if (seletorGraficos && seletorGraficos.value !== novoMes) seletorGraficos.value = novoMes;
  }

  // Modificar trocarMes e trocarMesGraficos para sincronizar
  const trocarMesOriginal = trocarMes;
  function trocarMes() {
  ...
  const trocarMesGraficosOriginal = trocarMesGraficos;
      ...
      card.innerHTML = `
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-center mb-2">
            <strong>${formatarNomeMes(mesId)}</strong>
